...
export default function MatePetProfile() {
  ...
  useEffect(() => {
    const readData = async () => {
      if (profileUserId && userId) {
        try {
          const response = await axios.get(`https://dolbosigae.site/member/petprofile/${profileUserId}`);
          setPetProfile(response.data);

          // 즐겨찾기 상태 확인
          const favResponse = await axios.get('https://dolbosigae.site/mate/fav/status', {
            params: {
              loginId: userId,
  ...
  const handleFavoriteClick = async () => {
    try {
      const response = await axios.post('https://dolbosigae.site/mate/fav', {
        loginId: userId,
        targetId: profileUserId
      });
