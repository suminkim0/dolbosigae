...
export default function MateSearch() {
  ...
  const readData = async () => {
    try {
      const response = await axios.get('https://dolbosigae.site/member/walkmates', {
        params: {
          page: currentPage,
        },
      });
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get('https://dolbosigae.site/member/walkmate/search', {
        params: {
  ...
  const handleChangeWalkProfile = async () => {
    try {
      await axios.post(`https://dolbosigae.site/walkmate/changeTF`, { Wid: selectedIds });
      readData();
      setSelectedIds([]);
    } catch (error) {
