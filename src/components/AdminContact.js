  ...
  const handlePageChange = (pageNo) => {
    ...
    if (searchResults) {
      ...
      const fetchPageData = async () => {
        try {
          const response = await axios.get(`https://dolbosigae.site/admin/search`, {
            params: {
              page: pageNo,
      ...
      const fetchPageData = async () => {
        try {
          const response = await axios.get(`https://dolbosigae.site/admin/contact?page=${pageNo}`);
          setAdminBoardList(response.data.admin);
          setPagination(response.data.pagination);
        } catch (error) {
  ...
  const deleteClick = async (adminNo, adminCommentCount) => {
    try {
      ...
      if (searchResults) {
        ...
        const fetchData = async () => {
          try {
            const response = await axios.get(`https://dolbosigae.site/admin/contact?page=${pagination.currentPage}`);
            setAdminBoardList(response.data.admin);
            setPagination(response.data.pagination);
          } catch (error) {
            console.error("There was an error fetching the page data!", error);
